import type { Style } from '@/schema'

type StyleInput = Omit<Style, 'verified' | 'sources'> & Partial<Pick<Style, 'verified' | 'sources'>>

/** All starter content is unverified; this only saves repeating the two fields. */
const s = (input: StyleInput): Style => ({ verified: false, sources: [], ...input })

/**
 * Classic grape × region styles. Profile ranges use the 1–5 scale
 * (1=low, 2=medium-, 3=medium, 4=medium+, 5=high). Tannin is null for whites.
 * Where the style is an appellation not in regions.ts, the nearest listed region is used and noted.
 */
export const styles: Style[] = [
  // ================================================================ WHITE
  s({
    id: 'sancerre',
    name: 'Sancerre',
    color: 'white',
    grapeIds: ['sauvignon-blanc'],
    regionId: 'sancerre',
    profile: {
      appearance: 'lemon-green',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [5, 5],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['gooseberry', 'grapefruit', 'grass', 'flint', 'elderflower'],
    oak: 'none',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, sauvignon-blanc grape profile, https://winefolly.com/grapes/sauvignon-blanc/ (hentet 2026-09-17)',
      'Wikipedia, Sancerre (wine), https://en.wikipedia.org/wiki/Sancerre_(wine) (hentet 2026-09-17)',
    ],
    story: [
      'Sancerre er den klassiske Sauvignon Blanc fra det østlige Loire. Vinen laves næsten altid på ståltank uden eg, så druens aromaer af stikkelsbær, græs og citrus står rent, og kalk- og flintjorden giver en tør, mineralsk finish.',
      'Sammenlignet med Marlborough er Sancerre mere afdæmpet og strammere, med mindre tropisk frugt. Den er referencen for den europæiske, kølige Sauvignon-stil.',
    ],
  }),
  s({
    id: 'marlborough-sauvignon-blanc',
    name: 'Marlborough Sauvignon Blanc',
    color: 'white',
    grapeIds: ['sauvignon-blanc'],
    regionId: 'marlborough',
    profile: {
      appearance: 'lemon-green',
      intensity: [4, 5],
      sweetness: [1, 1],
      acidity: [4, 5],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['passion-fruit', 'gooseberry', 'grapefruit', 'grass', 'tomato-leaf'],
    oak: 'none',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, sauvignon-blanc grape profile, https://winefolly.com/grapes/sauvignon-blanc/ (hentet 2026-09-17)',
      'Wikipedia, Marlborough wine region, https://en.wikipedia.org/wiki/Marlborough_wine_region (hentet 2026-09-17)',
    ],
    story: [
      "Marlborough Sauvignon Blanc er New Zealands signaturvin og en af de mest genkendelige hvidvine i verden. Stilen opstod i 1980'erne, da Cloudy Bay og andre viste, hvor eksplosivt aromatisk druen kunne blive i det solrige, kølige klima.",
      'Passionsfrugt, stikkelsbær og en grøn, urteagtig note er kendetegnene, sammen med høj syre og fuld aromaintensitet. Vinen laves på ståltank og drikkes ung.',
    ],
  }),
  s({
    id: 'bordeaux-blanc',
    name: 'Bordeaux Blanc',
    color: 'white',
    grapeIds: ['sauvignon-blanc', 'semillon'],
    regionId: 'bordeaux',
    profile: {
      appearance: 'lemon',
      intensity: [3, 3],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: null,
      alcohol: [3, 3],
      body: [3, 3],
      finish: [3, 3],
    },
    descriptorIds: ['lemon', 'grapefruit', 'wax', 'toast', 'grass'],
    oak: 'light',
    difficulty: 3,
    story: [
      'Tør hvid Bordeaux er en blanding af Sauvignon Blanc og Sémillon, hvor Sauvignon giver friskhed og aroma, og Sémillon giver fylde og lagringsevne. De bedste kommer fra Pessac-Léognan i Graves og gæres og lagres på egefade.',
      'Stilen er rundere og mere cremet end en ren Sauvignon Blanc, med noter af citrus, voks og let ristet eg.',
    ],
    note: 'Represents dry white Bordeaux (e.g. Pessac-Léognan), often barrel-fermented.',
  }),
  s({
    id: 'chablis',
    name: 'Chablis',
    color: 'white',
    grapeIds: ['chardonnay'],
    regionId: 'chablis',
    profile: {
      appearance: 'lemon',
      intensity: [2, 3],
      sweetness: [1, 1],
      acidity: [4, 5],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['green-apple', 'lemon', 'wet-stone', 'flint', 'chalk'],
    oak: 'none',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, chardonnay grape profile, https://winefolly.com/grapes/chardonnay/ (hentet 2026-09-17)',
      'Wikipedia, Chablis wine, https://en.wikipedia.org/wiki/Chablis_wine (hentet 2026-09-17)',
    ],
    story: [
      'Chablis er Chardonnay i sin strammeste form: kølig, tør og mineralsk med høj syre og grønt æble, citrus og en flintet, saltagtig karakter. Kimmeridge-kalken med fossile østers er en del af forklaringen.',
      'Klassisk Chablis laves uden ny eg, hvilket adskiller den tydeligt fra de fyldigere, nøddeagtige hvidvine fra Côte de Beaune. Premier og grand cru-vine kan dog lagres på ældre fade og udvikle mere fylde.',
    ],
  }),
  s({
    id: 'cote-de-beaune-blanc',
    name: 'Bourgogne Blanc (Côte de Beaune)',
    color: 'white',
    grapeIds: ['chardonnay'],
    regionId: 'cote-de-beaune',
    profile: {
      appearance: 'lemon',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: null,
      alcohol: [3, 4],
      body: [3, 4],
      finish: [4, 5],
    },
    descriptorIds: ['lemon', 'peach', 'nuts', 'butter', 'toast', 'flint'],
    oak: 'light',
    difficulty: 2,
    story: [
      'Hvid Bourgogne fra Côte de Beaune, med Meursault, Puligny-Montrachet og Chassagne-Montrachet i spidsen, er forbilledet for fadlagret Chardonnay. Vinen gæres og lagres på egefade med bærmekontakt (bâtonnage), hvilket giver fylde og noter af nødder, smør og ristet brød.',
      'Trods egen bevarer vinene en høj syre og en stenet mineralitet, som adskiller dem fra varmere landes Chardonnay.',
    ],
    note: 'Represents Meursault / Puligny-Montrachet / Chassagne-Montrachet.',
  }),
  s({
    id: 'napa-chardonnay',
    name: 'Napa Valley Chardonnay',
    color: 'white',
    grapeIds: ['chardonnay'],
    regionId: 'napa-valley',
    profile: {
      appearance: 'gold',
      intensity: [4, 5],
      sweetness: [1, 1],
      acidity: [3, 3],
      tannin: null,
      alcohol: [4, 5],
      body: [4, 5],
      finish: [3, 4],
    },
    descriptorIds: ['pineapple', 'peach', 'vanilla', 'butter', 'toast'],
    oak: 'pronounced',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, chardonnay grape profile, https://winefolly.com/grapes/chardonnay/ (hentet 2026-09-17)',
      'Wikipedia, Napa Valley AVA, https://en.wikipedia.org/wiki/Napa_Valley_AVA (hentet 2026-09-17)',
    ],
    story: [
      "Napa Valley Chardonnay er den klassiske New World-stil: moden frugt, fuld krop og tydelig ny eg. Den fyldige, smørrede stil blev populær i 1980'erne og 1990'erne og gav anledning til modreaktionen 'Anything But Chardonnay'.",
      'I dag er stilen ofte mere afbalanceret, men kendetegnene er stadig moden gul frugt, vanille og lavere syre end i Bourgogne.',
    ],
  }),
  s({
    id: 'margaret-river-chardonnay',
    name: 'Margaret River Chardonnay',
    color: 'white',
    grapeIds: ['chardonnay'],
    regionId: 'margaret-river',
    profile: {
      appearance: 'lemon',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: null,
      alcohol: [3, 4],
      body: [3, 4],
      finish: [4, 4],
    },
    descriptorIds: ['grapefruit', 'nectarine', 'peach', 'toast', 'flint'],
    oak: 'light',
    difficulty: 3,
    story: [
      "Margaret River Chardonnay er Australiens mest ansete Chardonnay-stil. Leeuwin Estates Art Series fra 1980'erne viste, at det maritime klima kunne give vine i Bourgogne-klassen.",
      'Stilen kombinerer moden frugt med høj syre og en præcis, integreret egekarakter, ofte med noter af grapefrugt, fersken og en flintet undertone.',
    ],
  }),
  s({
    id: 'mosel-riesling-kabinett',
    name: 'Mosel Riesling Kabinett',
    color: 'white',
    grapeIds: ['riesling'],
    regionId: 'mosel',
    profile: {
      appearance: 'lemon-green',
      intensity: [3, 4],
      sweetness: [2, 3],
      acidity: [5, 5],
      tannin: null,
      alcohol: [1, 2],
      body: [1, 2],
      finish: [3, 4],
    },
    descriptorIds: ['green-apple', 'lime', 'peach', 'wet-stone', 'honeysuckle'],
    oak: 'none',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, riesling grape profile, https://winefolly.com/grapes/riesling/ (hentet 2026-09-17)',
      'Wikipedia, German wine classification, https://en.wikipedia.org/wiki/German_wine_classification (hentet 2026-09-17)',
    ],
    story: [
      'Kabinett er det letteste niveau i det tyske Prädikat-system, lavet på druer høstet ved normal modenhed. Fra Mosel giver det vine med ofte kun 7–9 % alkohol (lovens minimum er 7 %), høj syre og let restsødme, der balancerer syren.',
      'Aromaerne er grønt æble, lime og hvide blomster med den skiferprægede mineralitet, som Mosel er kendt for. Det er en af de få klassiske stilarter med både lav alkohol og let sødme.',
    ],
  }),
  s({
    id: 'rheingau-riesling-trocken',
    name: 'Rheingau Riesling Trocken',
    color: 'white',
    grapeIds: ['riesling'],
    regionId: 'rheingau',
    profile: {
      appearance: 'lemon',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [5, 5],
      tannin: null,
      alcohol: [2, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['green-apple', 'lemon', 'peach', 'wet-stone', 'petrol'],
    oak: 'none',
    difficulty: 3,
    story: [
      'Tør Riesling fra Rheingau er den fyldigere modpol til Mosel. Rheingau har historisk været førende i den tørre tyske stil, og VDP-producenternes Grosses Gewächs fra de bedste marker er tørre vine med stor koncentration.',
      'Vinene har mere alkohol og krop end Mosel Kabinett, med moden frugt som fersken og æble, høj syre og en stram, stenet finish.',
    ],
  }),
  s({
    id: 'alsace-riesling',
    name: 'Alsace Riesling',
    color: 'white',
    grapeIds: ['riesling'],
    regionId: 'alsace',
    profile: {
      appearance: 'lemon',
      intensity: [3, 4],
      sweetness: [1, 2],
      acidity: [4, 5],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['lemon', 'green-apple', 'peach', 'wet-stone', 'petrol'],
    oak: 'none',
    difficulty: 3,
    story: [
      'Alsace Riesling er traditionelt tør og kraftigere end den tyske, med højere alkohol og fuld krop på grund af det tørre, solrige klima i Vogesernes regnskygge.',
      'Stilen er stram og mineralsk med citrus, moden pære og med alderen den klassiske petroleumsnote. Grand cru-marker som Schlossberg og Rangen er blandt de kendte.',
    ],
  }),
  s({
    id: 'clare-valley-riesling',
    name: 'Clare Valley Riesling',
    color: 'white',
    grapeIds: ['riesling'],
    regionId: 'clare-valley',
    profile: {
      appearance: 'lemon-green',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [5, 5],
      tannin: null,
      alcohol: [2, 3],
      body: [2, 2],
      finish: [3, 4],
    },
    descriptorIds: ['lime', 'lemon', 'green-apple', 'honeysuckle', 'petrol'],
    oak: 'none',
    difficulty: 2,
    story: [
      'Clare Valley Riesling er Australiens klassiske tørre Riesling, ofte med skruelåg, som Clare-producenterne var pionerer for i 2000. Vinene er tørre, lette og skarpt syrlige.',
      'Lime og citronskal er kendetegnene, og med alderen udvikler vinene noter af ristet brød og honning. Stilen er tørrere og mere limepræget end de tyske.',
    ],
  }),
  s({
    id: 'alsace-gewurztraminer',
    name: 'Alsace Gewürztraminer',
    color: 'white',
    grapeIds: ['gewurztraminer'],
    regionId: 'alsace',
    profile: {
      appearance: 'gold',
      intensity: [5, 5],
      sweetness: [1, 3],
      acidity: [1, 2],
      tannin: null,
      alcohol: [4, 5],
      body: [4, 5],
      finish: [3, 4],
    },
    descriptorIds: ['lychee', 'rose', 'ginger', 'honey', 'orange-peel'],
    oak: 'none',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, gewurztraminer grape profile, https://winefolly.com/grapes/gewurztraminer/ (hentet 2026-09-17)',
      'Wikipedia, Alsace wine, https://en.wikipedia.org/wiki/Alsace_wine (hentet 2026-09-17)',
    ],
    story: [
      'Gewürztraminer fra Alsace er en af verdens mest genkendelige hvidvine: dybt gylden, meget aromatisk med litchi, rose og krydderier, og med lav syre og høj alkohol. Ofte har den lidt restsødme.',
      'Alsace er druens klassiske hjem, og vinen serveres traditionelt til krydret mad, foie gras og Munster-ost.',
    ],
  }),
  s({
    id: 'alsace-pinot-gris',
    name: 'Alsace Pinot Gris',
    color: 'white',
    grapeIds: ['pinot-grigio'],
    regionId: 'alsace',
    profile: {
      appearance: 'gold',
      intensity: [3, 4],
      sweetness: [1, 3],
      acidity: [2, 3],
      tannin: null,
      alcohol: [4, 4],
      body: [4, 4],
      finish: [3, 4],
    },
    descriptorIds: ['pear', 'apricot', 'honey', 'ginger', 'smoke'],
    oak: 'none',
    difficulty: 3,
    story: [
      "Pinot Gris fra Alsace er en fyldig, ofte let sød hvidvin, langt fra den lette italienske Pinot Grigio. Indtil 2007 blev den solgt som Tokay d'Alsace.",
      'Stilen er rig med moden pære, honning og røg, moderat syre og høj alkohol. Den er ofte svær at skelne fra Gewürztraminer, men er mindre parfumeret.',
    ],
  }),
  s({
    id: 'muscat-d-alsace',
    name: "Muscat d'Alsace",
    color: 'white',
    grapeIds: ['muscat'],
    regionId: 'alsace',
    profile: {
      appearance: 'lemon-green',
      intensity: [5, 5],
      sweetness: [1, 1],
      acidity: [2, 3],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [2, 3],
    },
    descriptorIds: ['grape', 'orange-blossom', 'rose', 'elderflower'],
    oak: 'none',
    difficulty: 2,
    story: [
      "Muscat d'Alsace er en tør Muscat, hvilket er usædvanligt, da druen de fleste andre steder laves sød. Den laves på Muscat Blanc à Petits Grains og Muscat Ottonel.",
      'Vinen dufter intenst af friske druer og blomster, men er tør og let på ganen, og den serveres ofte til asparges.',
    ],
  }),
  s({
    id: 'pinot-grigio-veneto',
    name: 'Pinot Grigio (Veneto)',
    color: 'white',
    grapeIds: ['pinot-grigio'],
    regionId: 'veneto',
    profile: {
      appearance: 'lemon-green',
      intensity: [2, 2],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 2],
      finish: [2, 2],
    },
    descriptorIds: ['pear', 'lemon', 'green-apple', 'almond'],
    oak: 'none',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, pinot-grigio grape profile, https://winefolly.com/grapes/pinot-grigio/ (hentet 2026-09-17)',
    ],
    story: [
      "Pinot Grigio fra Veneto er den lette, friske og neutrale stil, som gjorde druen til en verdenssucces i 1990'erne og 2000'erne. Druerne høstes tidligt for at bevare syren og laves på ståltank.",
      'Stilen har lav aromaintensitet med citron, grøn pære og en let bitter mandelnote i finishen. Den er så anderledes end Alsace Pinot Gris, at man kan tro, det er to forskellige druer.',
    ],
    note: 'Represents the light delle Venezie style.',
  }),
  s({
    id: 'soave-classico',
    name: 'Soave Classico',
    color: 'white',
    grapeIds: ['garganega'],
    regionId: 'soave',
    profile: {
      appearance: 'lemon',
      intensity: [2, 3],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [2, 3],
    },
    descriptorIds: ['pear', 'lemon', 'melon', 'nuts', 'chalk'],
    oak: 'none',
    difficulty: 3,
    story: [
      'Soave Classico er hvidvinen fra de vulkanske bakker øst for Verona, lavet på Garganega. Stilen er tør og medium i krop med noter af mandel, hvide blomster og citrus og en let saltagtig mineralitet.',
      'Efter årtier som simpel supermarkedsvin har producenter som Pieropan og Inama vist, at Classico-bakkerne kan give komplekse vine med lagringspotentiale.',
    ],
  }),
  s({
    id: 'vouvray-sec',
    name: 'Vouvray Sec',
    color: 'white',
    grapeIds: ['chenin-blanc'],
    regionId: 'vouvray',
    profile: {
      appearance: 'lemon',
      intensity: [3, 4],
      sweetness: [1, 2],
      acidity: [5, 5],
      tannin: null,
      alcohol: [2, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['green-apple', 'quince', 'honey', 'wax', 'chalk'],
    oak: 'none',
    difficulty: 3,
    story: [
      'Tør Vouvray er Chenin Blanc fra tuffeau-kalken øst for Tours. Vinen har høj syre og en karakteristisk aroma af kvæde, æble, honning og lanolin, selv når den er helt tør.',
      'Stilen kan minde om Riesling i syre, men adskiller sig ved sin honning- og uldagtige note og en fyldigere tekstur.',
    ],
  }),
  s({
    id: 'stellenbosch-chenin-blanc',
    name: 'Stellenbosch Chenin Blanc',
    color: 'white',
    grapeIds: ['chenin-blanc'],
    regionId: 'stellenbosch',
    profile: {
      appearance: 'lemon',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [4, 5],
      tannin: null,
      alcohol: [3, 4],
      body: [3, 3],
      finish: [3, 3],
    },
    descriptorIds: ['green-apple', 'pineapple', 'peach', 'honey'],
    oak: 'none',
    difficulty: 3,
    story: [
      "Sydafrikansk Chenin Blanc fra gamle buskvinstokke er blevet landets mest ansete hvidvin. Producenter som Ken Forrester og Alheit satte fra 2000'erne stilen med koncentrerede, ofte fadlagrede vine.",
      'Sammenlignet med Loire er vinene fyldigere og mere frugtige, med moden gul frugt, honning og ofte en let egekarakter, men stadig med høj syre.',
    ],
  }),
  s({
    id: 'condrieu',
    name: 'Condrieu',
    color: 'white',
    grapeIds: ['viognier'],
    regionId: 'northern-rhone',
    profile: {
      appearance: 'gold',
      intensity: [4, 5],
      sweetness: [1, 1],
      acidity: [2, 3],
      tannin: null,
      alcohol: [4, 5],
      body: [4, 5],
      finish: [3, 4],
    },
    descriptorIds: ['apricot', 'peach', 'honeysuckle', 'orange-blossom'],
    oak: 'light',
    difficulty: 2,
    story: [
      "Condrieu er Viogniers hjem, en lille appellation på stejle granitterrasser i det nordlige Rhône. I 1960'erne var der kun få hektar tilbage, men vinen er siden blevet en af Frankrigs mest eftertragtede hvidvine.",
      'Stilen er fyldig og parfumeret med abrikos, fersken og hvide blomster, lav syre og høj alkohol. Den drikkes typisk ung.',
    ],
    note: 'Condrieu is an appellation within the Northern Rhône; not listed separately in regions.ts.',
  }),
  s({
    id: 'rias-baixas-albarino',
    name: 'Rías Baixas Albariño',
    color: 'white',
    grapeIds: ['albarino'],
    regionId: 'rias-baixas',
    profile: {
      appearance: 'lemon-green',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [5, 5],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [3, 3],
    },
    descriptorIds: ['grapefruit', 'lemon', 'peach', 'saline'],
    oak: 'none',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, albarino grape profile, https://winefolly.com/grapes/albarino/ (hentet 2026-09-17)',
      'Wikipedia, Rías Baixas (DO), https://en.wikipedia.org/wiki/Rías_Baixas_(DO) (hentet 2026-09-17)',
    ],
    story: [
      'Albariño fra Rías Baixas er den friske, aromatiske hvidvin fra Galiciens atlanterhavskyst. Vinen laves på ståltank og har høj syre, moderat alkohol og noter af citrus, fersken og en saltagtig karakter.',
      'Den tykke druehud giver en let bitterhed og tekstur, som adskiller den fra Sauvignon Blanc, selvom aromaintensiteten kan ligne.',
    ],
  }),
  s({
    id: 'wachau-gruner-veltliner',
    name: 'Wachau Grüner Veltliner',
    color: 'white',
    grapeIds: ['gruner-veltliner'],
    regionId: 'wachau',
    profile: {
      appearance: 'lemon-green',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [4, 5],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['green-apple', 'grapefruit', 'white-pepper', 'wet-stone'],
    oak: 'none',
    difficulty: 2,
    story: [
      'Grüner Veltliner fra Wachau er Østrigs signaturvin. Federspiel-niveauet giver lette, tørre vine, mens Smaragd fra de bedste terrasser giver fyldige, kraftige vine med lagringspotentiale.',
      'Kendetegnet er hvid peber sammen med grønt æble, citrus og en stenet mineralitet. Vinene har høj syre og laves typisk uden ny eg.',
    ],
  }),

  // ================================================================ SWEET
  s({
    id: 'sauternes',
    name: 'Sauternes',
    color: 'sweet',
    grapeIds: ['semillon', 'sauvignon-blanc'],
    regionId: 'sauternes',
    profile: {
      appearance: 'gold',
      intensity: [5, 5],
      sweetness: [5, 5],
      acidity: [4, 4],
      tannin: null,
      alcohol: [3, 4],
      body: [4, 5],
      finish: [5, 5],
    },
    descriptorIds: ['apricot', 'honey', 'orange-peel', 'dried-fruit', 'vanilla'],
    oak: 'light',
    difficulty: 2,
    story: [
      'Sauternes er verdens mest berømte søde vin, lavet på Sémillon og Sauvignon Blanc angrebet af ædelråd (botrytis). Svampen tørrer druerne ind og koncentrerer sukker, syre og aroma.',
      'Vinen er dybt gylden, fuld i krop og sød, men den høje syre holder den frisk. Honning, abrikos, marmelade og en safranagtig botrytis-note er kendetegnene, og de bedste kan lagres i mange årtier.',
    ],
    note: 'Botrytised sweet wine. Included so Sémillon has a classic style; sweet wines are otherwise phase 10.',
  }),

  // ================================================================ RED
  s({
    id: 'pauillac',
    name: 'Pauillac',
    color: 'red',
    grapeIds: ['cabernet-sauvignon', 'merlot'],
    regionId: 'pauillac',
    profile: {
      appearance: 'ruby',
      intensity: [4, 5],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: [4, 5],
      alcohol: [3, 4],
      body: [4, 5],
      finish: [4, 5],
    },
    descriptorIds: ['blackcurrant', 'cedar', 'tobacco', 'green-bell-pepper', 'toast'],
    oak: 'pronounced',
    difficulty: 2,
    story: [
      'Pauillac er den mest strukturerede stil i Bordeaux, domineret af Cabernet Sauvignon fra Médocs dybe grusbanker. Tre af 1855-klassifikationens fem premiers crus ligger her.',
      'Kendetegnene er solbær, cedertræ, blyant og tobak, høj tannin og syre og en lang finish. Vinene lagres på ny fransk eg og kræver typisk 10–20 år for at vise sig fra deres bedste side.',
    ],
  }),
  s({
    id: 'napa-cabernet-sauvignon',
    name: 'Napa Valley Cabernet Sauvignon',
    color: 'red',
    grapeIds: ['cabernet-sauvignon'],
    regionId: 'napa-valley',
    profile: {
      appearance: 'ruby',
      intensity: [5, 5],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: [4, 5],
      alcohol: [4, 5],
      body: [5, 5],
      finish: [4, 5],
    },
    descriptorIds: ['blackcurrant', 'black-cherry', 'vanilla', 'chocolate', 'mint'],
    oak: 'pronounced',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, cabernet-sauvignon grape profile, https://winefolly.com/grapes/cabernet-sauvignon/ (hentet 2026-09-17)',
      'Wikipedia, Napa Valley AVA, https://en.wikipedia.org/wiki/Napa_Valley_AVA (hentet 2026-09-17)',
    ],
    story: [
      'Napa Valley Cabernet Sauvignon er den New World-stil, som slog Bordeaux i Paris i 1976. Det varme, solrige klima giver fuldt modne druer og vine med mere frugt, mere alkohol og blødere tannin end Bordeaux.',
      'Sort solbær og brombær, vanille fra ny eg og ofte en note af mynte eller chokolade er kendetegnene. Stilen er fuld i krop og drikkes gerne yngre end Pauillac.',
    ],
  }),
  s({
    id: 'margaret-river-cabernet-sauvignon',
    name: 'Margaret River Cabernet Sauvignon',
    color: 'red',
    grapeIds: ['cabernet-sauvignon', 'merlot'],
    regionId: 'margaret-river',
    profile: {
      appearance: 'ruby',
      intensity: [4, 4],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: [4, 4],
      alcohol: [3, 4],
      body: [4, 4],
      finish: [4, 4],
    },
    descriptorIds: ['blackcurrant', 'cedar', 'mint', 'tobacco', 'black-plum'],
    oak: 'light',
    difficulty: 3,
    story: [
      'Margaret River Cabernet er Australiens mest Bordeaux-lignende rødvin, ofte blandet med Merlot. Det maritime klima giver vine med både modenhed og friskhed.',
      'Stilen ligger mellem Bordeaux og Napa: solbær, cedertræ og en urteagtig, ofte let grøn note, med markant, men fin tannin. Cullen, Moss Wood og Vasse Felix er blandt de klassiske producenter.',
    ],
  }),
  s({
    id: 'maipo-cabernet-sauvignon',
    name: 'Maipo Cabernet Sauvignon',
    color: 'red',
    grapeIds: ['cabernet-sauvignon'],
    regionId: 'maipo-valley',
    profile: {
      appearance: 'ruby',
      intensity: [4, 5],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: [4, 4],
      alcohol: [4, 4],
      body: [4, 4],
      finish: [3, 4],
    },
    descriptorIds: ['blackcurrant', 'mint', 'eucalyptus', 'black-plum', 'vanilla'],
    oak: 'light',
    difficulty: 3,
    story: [
      "Maipo Cabernet Sauvignon er Chiles klassiske rødvin, med rødder i de franske druer, som blev indført i 1800-tallet. De bedste vine kommer fra Alto Maipo ved Andes' fod.",
      'En mentol- eller eukalyptus-note er stilens kendemærke, sammen med solbær, en urteagtig karakter og fastere tannin end i Napa. Vinene er typisk lidt lettere og mere friske end de californiske.',
    ],
  }),
  s({
    id: 'stellenbosch-cabernet-sauvignon',
    name: 'Stellenbosch Cabernet Sauvignon',
    color: 'red',
    grapeIds: ['cabernet-sauvignon'],
    regionId: 'stellenbosch',
    profile: {
      appearance: 'ruby',
      intensity: [4, 5],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: [4, 4],
      alcohol: [4, 4],
      body: [4, 4],
      finish: [4, 4],
    },
    descriptorIds: ['blackcurrant', 'black-plum', 'cedar', 'smoke', 'dried-herbs'],
    oak: 'pronounced',
    difficulty: 3,
    story: [
      'Stellenbosch Cabernet er Sydafrikas førende rødvinsstil, dyrket på granit- og sandstensjorden ved Simonsberg og Helderberg. Vinen ligger stilmæssigt mellem Bordeaux og New World.',
      'Solbær, cedertræ og en karakteristisk røget eller urteagtig note er typisk, med markant tannin og moderat alkohol.',
    ],
  }),
  s({
    id: 'saint-emilion',
    name: 'Saint-Émilion',
    color: 'red',
    grapeIds: ['merlot', 'cabernet-franc'],
    regionId: 'saint-emilion',
    profile: {
      appearance: 'ruby',
      intensity: [4, 4],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: [3, 4],
      alcohol: [3, 4],
      body: [4, 4],
      finish: [4, 4],
    },
    descriptorIds: ['red-plum', 'black-plum', 'cedar', 'tobacco', 'chocolate'],
    oak: 'pronounced',
    difficulty: 3,
    story: [
      "Saint-Émilion er Bordeaux' højre bred i et glas: Merlot-domineret med Cabernet Franc, blødere og rundere end Pauillac og tilgængelig tidligere. De bedste vine kommer fra kalkplateauet ved byen.",
      'Rød blomme, kirsebær, chokolade og en urteagtig, floral note fra Cabernet Franc er kendetegnene, sammen med medium-plus tannin og lavere syre end på venstre bred.',
    ],
  }),
  s({
    id: 'napa-merlot',
    name: 'Napa Valley Merlot',
    color: 'red',
    grapeIds: ['merlot'],
    regionId: 'napa-valley',
    profile: {
      appearance: 'ruby',
      intensity: [4, 4],
      sweetness: [1, 1],
      acidity: [3, 3],
      tannin: [3, 4],
      alcohol: [4, 5],
      body: [4, 5],
      finish: [3, 4],
    },
    descriptorIds: ['black-plum', 'black-cherry', 'chocolate', 'vanilla', 'coffee'],
    oak: 'pronounced',
    difficulty: 3,
    story: [
      "Napa Valley Merlot er en fyldig, moden og blød rødvin, som blev meget populær i USA i 1990'erne. Filmen Sideways fra 2004 gav druen et imageproblem, men stilen har bevaret sin plads.",
      'Sort blomme, kirsebær, chokolade og vanille fra eg er typisk, med bløde tanniner, høj alkohol og fuld krop.',
    ],
  }),
  s({
    id: 'cote-de-nuits-rouge',
    name: 'Bourgogne Rouge (Côte de Nuits)',
    color: 'red',
    grapeIds: ['pinot-noir'],
    regionId: 'cote-de-nuits',
    profile: {
      appearance: 'ruby',
      intensity: [3, 3],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: [2, 3],
      alcohol: [3, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['red-cherry', 'raspberry', 'forest-floor', 'violet', 'mushroom'],
    oak: 'light',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, pinot-noir grape profile, https://winefolly.com/grapes/pinot-noir/ (hentet 2026-09-17)',
    ],
    story: [
      'Rød Bourgogne fra Côte de Nuits er Pinot Noirs klassiske udtryk: lys i farven, men intens i duft og smag. Landsbyer som Gevrey-Chambertin, Chambolle-Musigny og Vosne-Romanée har hver deres karakter.',
      'Rødt kirsebær, hindbær og med alderen skovbund og svampe er kendetegnene, sammen med høj syre og silkeagtig, moderat tannin. Vinene lagres på egefade, men egen skal være diskret.',
    ],
    note: 'Represents Gevrey-Chambertin / Nuits-Saint-Georges / Vosne-Romanée village and premier cru wines.',
  }),
  s({
    id: 'willamette-pinot-noir',
    name: 'Willamette Valley Pinot Noir',
    color: 'red',
    grapeIds: ['pinot-noir'],
    regionId: 'willamette-valley',
    profile: {
      appearance: 'ruby',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: [2, 3],
      alcohol: [3, 4],
      body: [3, 3],
      finish: [3, 4],
    },
    descriptorIds: ['red-cherry', 'cranberry', 'raspberry', 'forest-floor', 'vanilla'],
    oak: 'light',
    difficulty: 3,
    story: [
      'Willamette Valley Pinot Noir er den amerikanske stil, der ligger tættest på Bourgogne. Oregons kølige, regnfulde klima giver vine med friskhed og elegance snarere end kraft.',
      'Rødt kirsebær, tranebær og en jordet, krydret note er typisk, med høj syre og let tannin. Vinene er ofte lidt mere frugtige end Bourgogne, men tydeligt mere afdæmpede end Central Otago.',
    ],
  }),
  s({
    id: 'central-otago-pinot-noir',
    name: 'Central Otago Pinot Noir',
    color: 'red',
    grapeIds: ['pinot-noir'],
    regionId: 'central-otago',
    profile: {
      appearance: 'ruby',
      intensity: [4, 4],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: [3, 3],
      alcohol: [3, 4],
      body: [3, 4],
      finish: [3, 4],
    },
    descriptorIds: ['black-cherry', 'raspberry', 'thyme', 'vanilla', 'red-plum'],
    oak: 'light',
    difficulty: 2,
    story: [
      'Central Otago Pinot Noir er den mørkeste og frugtigste af de klassiske Pinot Noir-stilarter. Det kontinentale klima med varme dage og kolde nætter giver fuld modenhed og høj syre på samme tid.',
      'Mørkt kirsebær, blomme og en krydret, urteagtig note er kendetegnene, med mere farve og alkohol end Bourgogne og en tydelig frugtsødme.',
    ],
  }),
  s({
    id: 'yarra-valley-pinot-noir',
    name: 'Yarra Valley Pinot Noir',
    color: 'red',
    grapeIds: ['pinot-noir'],
    regionId: 'yarra-valley',
    profile: {
      appearance: 'ruby',
      intensity: [3, 3],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: [2, 3],
      alcohol: [3, 3],
      body: [3, 3],
      finish: [3, 3],
    },
    descriptorIds: ['red-cherry', 'strawberry', 'dried-herbs', 'vanilla'],
    oak: 'light',
    difficulty: 3,
    story: [
      "Yarra Valley Pinot Noir er Australiens klassiske kølige Pinot Noir, lavet i bakkerne øst for Melbourne. Området blev genoplivet fra 1960'erne netop med Pinot Noir og Chardonnay for øje.",
      'Stilen er lys, frisk og elegant med rødt kirsebær, jordbær og en let krydret note, moderat alkohol og fin tannin.',
    ],
  }),
  s({
    id: 'cote-rotie',
    name: 'Côte-Rôtie',
    color: 'red',
    grapeIds: ['syrah'],
    regionId: 'northern-rhone',
    profile: {
      appearance: 'ruby',
      intensity: [4, 5],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: [4, 4],
      alcohol: [3, 4],
      body: [4, 4],
      finish: [4, 5],
    },
    descriptorIds: ['blackberry', 'black-pepper', 'violet', 'bacon', 'smoke', 'liquorice'],
    oak: 'light',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, syrah grape profile, https://winefolly.com/grapes/syrah/ (hentet 2026-09-17)',
      'Wikipedia, Côte-Rôtie AOC, https://en.wikipedia.org/wiki/Côte-Rôtie_AOC (hentet 2026-09-17)',
    ],
    story: [
      "Côte-Rôtie, 'den ristede skråning', er den nordligste appellation i Rhône og laver Syrah på stejle terrasser, traditionelt gæret sammen med lidt Viognier, som giver duft og farvestabilitet.",
      'Stilen er den elegante Syrah: sort peber, violer, bacon og mørke bær, med fast tannin og høj syre. Sammenlignet med Barossa Shiraz er den lettere, mere krydret og mindre sød i frugten.',
    ],
    note: 'Represents Northern Rhône Syrah in general (Hermitage, Saint-Joseph, Crozes-Hermitage).',
  }),
  s({
    id: 'barossa-shiraz',
    name: 'Barossa Shiraz',
    color: 'red',
    grapeIds: ['syrah'],
    regionId: 'barossa-valley',
    profile: {
      appearance: 'purple',
      intensity: [5, 5],
      sweetness: [1, 1],
      acidity: [3, 3],
      tannin: [4, 4],
      alcohol: [4, 5],
      body: [5, 5],
      finish: [4, 4],
    },
    descriptorIds: ['blackberry', 'black-plum', 'chocolate', 'vanilla', 'liquorice', 'eucalyptus'],
    oak: 'pronounced',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, syrah grape profile, https://winefolly.com/grapes/syrah/ (hentet 2026-09-17)',
      'Wikipedia, Barossa Valley, https://en.wikipedia.org/wiki/Barossa_Valley (hentet 2026-09-17)',
    ],
    story: [
      "Barossa Shiraz er Australiens ikoniske rødvin, ofte fra gamle stokke i det varme, tørre Barossa Valley. Penfolds Grange satte standarden i 1950'erne, og vinen har været kendt for kraft og frugtsødme lige siden.",
      'Brombær, sort blomme, chokolade og eukalyptus, ofte med vanille og kokos fra amerikansk eg, er kendetegnene, sammen med høj alkohol, fuld krop og bløde tanniner.',
    ],
  }),
  s({
    id: 'chateauneuf-du-pape',
    name: 'Châteauneuf-du-Pape',
    color: 'red',
    grapeIds: ['grenache', 'syrah'],
    regionId: 'chateauneuf-du-pape',
    profile: {
      appearance: 'ruby',
      intensity: [4, 4],
      sweetness: [1, 1],
      acidity: [3, 3],
      tannin: [3, 4],
      alcohol: [5, 5],
      body: [4, 5],
      finish: [4, 4],
    },
    descriptorIds: [
      'strawberry',
      'raspberry',
      'dried-herbs',
      'black-pepper',
      'liquorice',
      'leather',
    ],
    oak: 'light',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, grenache grape profile, https://winefolly.com/grapes/grenache/ (hentet 2026-09-17)',
      'Wikipedia, Châteauneuf-du-Pape AOC, https://en.wikipedia.org/wiki/Châteauneuf-du-Pape_AOC (hentet 2026-09-17)',
    ],
    story: [
      'Châteauneuf-du-Pape er det sydlige Rhônes store rødvin, en Grenache-baseret blanding med Syrah, Mourvèdre og op til flere andre druer. De store, runde sten i vinmarkerne oplagrer varme og giver kraftige vine.',
      'Stilen er varm og fyldig med rød og mørk frugt, tørrede urter (garrigue), lakrids og læder, høj alkohol, moderat syre og blød tannin. Ny eg bruges sjældent.',
    ],
    note: 'Mourvèdre is also a principal grape but is not in the starter grape list.',
  }),
  s({
    id: 'priorat',
    name: 'Priorat',
    color: 'red',
    grapeIds: ['grenache'],
    regionId: 'priorat',
    profile: {
      appearance: 'ruby',
      intensity: [5, 5],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: [4, 5],
      alcohol: [5, 5],
      body: [5, 5],
      finish: [4, 5],
    },
    descriptorIds: ['black-cherry', 'blackberry', 'liquorice', 'wet-stone', 'toast'],
    oak: 'pronounced',
    difficulty: 3,
    story: [
      "Priorat er en koncentreret, mineralsk rødvin fra Cataloniens skiferbjerge, lavet på gamle Garnacha- og Cariñena-stokke. Stilen blev skabt i slutningen af 1980'erne af en gruppe pionerer, som genoplivede det næsten forladte område.",
      'Mørke bær, lakrids og en tydelig stenet, skiferpræget karakter er kendetegnene, sammen med høj alkohol, fast tannin og ofte ny fransk eg.',
    ],
    note: 'Cariñena (Carignan) is the other principal grape but is not in the starter grape list.',
  }),
  s({
    id: 'rioja-reserva',
    name: 'Rioja Reserva',
    color: 'red',
    grapeIds: ['tempranillo', 'grenache'],
    regionId: 'rioja',
    profile: {
      appearance: 'garnet',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: [3, 4],
      alcohol: [4, 4],
      body: [3, 4],
      finish: [4, 4],
    },
    descriptorIds: [
      'red-cherry',
      'vanilla',
      'coconut',
      'dill',
      'leather',
      'tobacco',
      'dried-fruit',
    ],
    oak: 'pronounced',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, tempranillo grape profile, https://winefolly.com/grapes/tempranillo/ (hentet 2026-09-17)',
      'Wikipedia, Rioja (wine), https://en.wikipedia.org/wiki/Rioja_(wine) (hentet 2026-09-17)',
    ],
    story: [
      'Rioja Reserva er den klassiske spanske fadlagrede rødvin: mindst tre års lagring, heraf mindst ét år på egefad, før den sælges. Traditionen med amerikansk eg stammer fra 1800-tallets Bordeaux-inspirerede handelshuse.',
      'Rødt kirsebær og blomme sammen med vanille, kokos, dild og tobak fra egen er kendetegnene. Tempranillo, ofte med Garnacha, giver medium krop og syre og bløde, modne tanniner.',
    ],
  }),
  s({
    id: 'ribera-del-duero',
    name: 'Ribera del Duero',
    color: 'red',
    grapeIds: ['tempranillo'],
    regionId: 'ribera-del-duero',
    profile: {
      appearance: 'ruby',
      intensity: [5, 5],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: [4, 5],
      alcohol: [4, 5],
      body: [5, 5],
      finish: [4, 4],
    },
    descriptorIds: ['black-plum', 'blackberry', 'vanilla', 'toast', 'liquorice'],
    oak: 'pronounced',
    difficulty: 2,
    story: [
      'Ribera del Duero er Tempranillo i mørkere og kraftigere form end Rioja, fra det høje, kontinentale plateau ved Duero. Vega Sicilia og Pesquera skabte områdets ry.',
      'Stilen er dybfarvet med sort frugt, chokolade og røg, fuld krop, høj alkohol og markant tannin. Fransk eg er mere almindelig end amerikansk, så vanillen er mindre fremtrædende end i Rioja.',
    ],
  }),
  s({
    id: 'chianti-classico',
    name: 'Chianti Classico',
    color: 'red',
    grapeIds: ['sangiovese'],
    regionId: 'chianti-classico',
    profile: {
      appearance: 'ruby',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [4, 5],
      tannin: [4, 4],
      alcohol: [3, 4],
      body: [3, 4],
      finish: [3, 4],
    },
    descriptorIds: ['red-cherry', 'red-plum', 'dried-herbs', 'tomato-leaf', 'leather'],
    oak: 'light',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, sangiovese grape profile, https://winefolly.com/grapes/sangiovese/ (hentet 2026-09-17)',
      'Wikipedia, Chianti, https://en.wikipedia.org/wiki/Chianti (hentet 2026-09-17)',
      'Consorzio Vino Chianti Classico, disciplinare (min. 80 % Sangiovese; Riserva 24 mdr., Gran Selezione 30 mdr.), https://www.chianticlassico.com/wp-content/uploads/2020/11/disciplinare-Chianti-Classico.pdf',
    ],
    story: [
      'Chianti Classico er Sangiovese fra det historiske område mellem Firenze og Siena, mindst 80 % Sangiovese og genkendelig på den sorte hane på flaskehalsen. Den strå-omvundne flaske hører fortiden til.',
      'Surkirsebær, tørrede urter, tomatblad og læder er kendetegnene, sammen med høj syre og fast tannin. Stilen er lettere i farve og krop end Brunello og udpræget madvenlig.',
    ],
  }),
  s({
    id: 'brunello-di-montalcino',
    name: 'Brunello di Montalcino',
    color: 'red',
    grapeIds: ['sangiovese'],
    regionId: 'brunello-di-montalcino',
    profile: {
      appearance: 'garnet',
      intensity: [4, 4],
      sweetness: [1, 1],
      acidity: [4, 5],
      tannin: [4, 5],
      alcohol: [4, 4],
      body: [4, 5],
      finish: [5, 5],
    },
    descriptorIds: ['red-cherry', 'black-cherry', 'leather', 'tobacco', 'dried-herbs'],
    oak: 'light',
    difficulty: 3,
    story: [
      'Brunello di Montalcino er 100 % Sangiovese fra det varme, tørre Montalcino, lagret mindst fem år før salg. Vinen er Toscanas mest kraftfulde og langtidsholdbare udtryk for druen.',
      'Stilen er fyldigere og mere tanninrig end Chianti Classico, med mørkere kirsebær, tørrede blomster, læder og balsamiske noter fra den lange fadlagring.',
    ],
  }),
  s({
    id: 'barolo',
    name: 'Barolo',
    color: 'red',
    grapeIds: ['nebbiolo'],
    regionId: 'barolo',
    profile: {
      appearance: 'garnet',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [4, 5],
      tannin: [5, 5],
      alcohol: [4, 5],
      body: [4, 5],
      finish: [5, 5],
    },
    descriptorIds: ['rose', 'tar', 'red-cherry', 'liquorice', 'dried-herbs'],
    oak: 'light',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, nebbiolo grape profile, https://winefolly.com/grapes/nebbiolo/ (hentet 2026-09-17)',
      'Wikipedia, Barolo, https://en.wikipedia.org/wiki/Barolo (hentet 2026-09-17)',
      'Quattrocalici, Barolo DOCG disciplinare (38 mdr., heraf 18 i træ; Riserva 62 mdr.), https://www.quattrocalici.it/denominazioni/barolo-docg/',
    ],
    story: [
      "Barolo er Nebbiolo fra bakkerne sydvest for Alba, lagret mindst 38 måneder før salg. Vinens tilnavn, 'kongernes vin', stammer fra dens popularitet ved Savoyen-hoffet i 1800-tallet.",
      'Den lyse, granatrøde farve narrer: vinen har meget høj tannin og syre. Roser, tjære, tørrede kirsebær og lakrids er de klassiske aromaer. Sammen med Barbaresco er det en af de få stilarter, hvor lys farve og kraftig struktur følges ad.',
    ],
  }),
  s({
    id: 'barbera-d-asti',
    name: "Barbera d'Asti",
    color: 'red',
    grapeIds: ['barbera'],
    regionId: 'piemonte',
    profile: {
      appearance: 'ruby',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [5, 5],
      tannin: [2, 3],
      alcohol: [3, 4],
      body: [3, 3],
      finish: [3, 3],
    },
    descriptorIds: ['red-cherry', 'red-plum', 'blackberry', 'black-pepper'],
    oak: 'none',
    difficulty: 3,
    story: [
      "Barbera d'Asti er Piemontes hverdagsrødvin i sin bedste form: dyb farve, høj syre og lav tannin, en usædvanlig kombination, som gør den saftig og madvenlig.",
      'Rødt og sort kirsebær med en krydret note er kendetegnene. Nogle producenter lagrer på ny eg, som tilføjer vanille og struktur, men de fleste holder stilen frisk og frugtig.',
    ],
    note: 'Asti is not a separate entry in regions.ts; mapped to Piemonte.',
  }),
  s({
    id: 'mendoza-malbec',
    name: 'Mendoza Malbec',
    color: 'red',
    grapeIds: ['malbec'],
    regionId: 'mendoza',
    profile: {
      appearance: 'purple',
      intensity: [5, 5],
      sweetness: [1, 1],
      acidity: [2, 3],
      tannin: [3, 4],
      alcohol: [4, 5],
      body: [4, 5],
      finish: [3, 4],
    },
    descriptorIds: ['blackberry', 'black-plum', 'violet', 'vanilla', 'chocolate'],
    oak: 'pronounced',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, malbec grape profile, https://winefolly.com/grapes/malbec/ (hentet 2026-09-17)',
      'Wikipedia, Mendoza wine, https://en.wikipedia.org/wiki/Mendoza_wine (hentet 2026-09-17)',
    ],
    story: [
      'Mendoza Malbec er Argentinas nationalvin. Druen kom fra Frankrig i 1853 og blomstrede i Andesbjergenes tørre, solrige højde, hvor den giver mørkere og blødere vine end i Cahors.',
      'Dyb violet farve, sort blomme, brombær og violer er kendetegnene, sammen med fuld krop, høj alkohol og runde tanniner, ofte med vanille fra eg. Vine fra Uco-dalen er friskere og mere blomstrede end dem fra lavere områder.',
    ],
  }),
  s({
    id: 'sonoma-zinfandel',
    name: 'Sonoma Zinfandel',
    color: 'red',
    grapeIds: ['zinfandel'],
    regionId: 'sonoma',
    profile: {
      appearance: 'ruby',
      intensity: [5, 5],
      sweetness: [1, 2],
      acidity: [3, 3],
      tannin: [3, 4],
      alcohol: [5, 5],
      body: [5, 5],
      finish: [4, 4],
    },
    descriptorIds: ['blackberry', 'raspberry', 'black-pepper', 'dried-fruit', 'vanilla'],
    oak: 'light',
    difficulty: 2,
    story: [
      'Sonoma Zinfandel er Californiens egen rødvin, ofte fra gamle stokke i Dry Creek Valley og Russian River, nogle plantet før 1900. Stilen er kraftig, frugtig og alkoholstærk.',
      'Brombær, hindbærsyltetøj, sort peber og en varm, krydret note er kendetegnene, sammen med høj alkohol, fuld krop og moderat tannin. Den er tæt beslægtet med Primitivo fra Puglia.',
    ],
    note: 'Represents Dry Creek Valley Zinfandel.',
  }),
  s({
    id: 'primitivo-di-manduria',
    name: 'Primitivo di Manduria',
    color: 'red',
    grapeIds: ['zinfandel'],
    regionId: 'puglia',
    profile: {
      appearance: 'ruby',
      intensity: [5, 5],
      sweetness: [1, 2],
      acidity: [3, 3],
      tannin: [3, 4],
      alcohol: [5, 5],
      body: [5, 5],
      finish: [3, 4],
    },
    descriptorIds: ['black-plum', 'dried-fruit', 'blackberry', 'liquorice', 'chocolate'],
    oak: 'light',
    difficulty: 3,
    story: [
      'Primitivo di Manduria er Puglias kraftige rødvin fra den samme drue som Zinfandel. Navnet Primitivo hentyder til, at druen modner tidligt. Vinen laves i det varme, flade Salento nær Taranto.',
      'Moden mørk frugt, tørrede figner, krydderier og en tydelig frugtsødme er kendetegnene, sammen med høj alkohol og bløde tanniner. Ofte er den lidt sødere og mere syltetøjsagtig end Sonoma Zinfandel.',
    ],
    note: 'Same grape as Zinfandel; deliberately close to Sonoma Zinfandel and hard to separate.',
  }),
  s({
    id: 'beaujolais',
    name: 'Beaujolais',
    color: 'red',
    grapeIds: ['gamay'],
    regionId: 'beaujolais',
    profile: {
      appearance: 'purple',
      intensity: [3, 3],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: [2, 2],
      alcohol: [2, 3],
      body: [2, 2],
      finish: [2, 3],
    },
    descriptorIds: ['raspberry', 'red-cherry', 'banana', 'violet', 'cranberry'],
    oak: 'none',
    difficulty: 1,
    verified: true,
    sources: [
      'Wine Folly, gamay grape profile, https://winefolly.com/grapes/gamay/ (hentet 2026-09-17)',
      'Wikipedia, Beaujolais, https://en.wikipedia.org/wiki/Beaujolais (hentet 2026-09-17)',
    ],
    story: [
      'Beaujolais er Gamay fra granitbakkerne syd for Bourgogne, typisk lavet med kulsyremaceration, som giver frisk frugt og bløde tanniner. Beaujolais Nouveau, frigivet i november, gjorde stilen verdenskendt.',
      'Rødt kirsebær, hindbær og en karakteristisk banan- eller tyggegummiagtig note fra macerationen er kendetegnene, sammen med høj syre, lav tannin og let krop. Cru-vinene er alvorligere og mere jordede.',
    ],
    note: 'Represents Beaujolais / Beaujolais-Villages made with carbonic maceration.',
  }),
  s({
    id: 'chinon',
    name: 'Chinon',
    color: 'red',
    grapeIds: ['cabernet-franc'],
    regionId: 'loire',
    profile: {
      appearance: 'ruby',
      intensity: [3, 3],
      sweetness: [1, 1],
      acidity: [4, 5],
      tannin: [3, 3],
      alcohol: [2, 3],
      body: [2, 3],
      finish: [3, 3],
    },
    descriptorIds: ['raspberry', 'redcurrant', 'green-bell-pepper', 'violet', 'wet-stone'],
    oak: 'none',
    difficulty: 3,
    story: [
      "Chinon er Loires klassiske Cabernet Franc, en let til medium rødvin fra det kølige klima omkring Rabelais' fødeby. Sand- og grusjord ved floden giver lette vine, mens kalkskrænterne giver mere struktur.",
      'Hindbær, grøn peberfrugt, blyant og violer er kendetegnene, sammen med høj syre og moderat tannin. Vinen serveres ofte let kølig.',
    ],
    note: 'Chinon is not a separate entry in regions.ts; mapped to Loire.',
  }),
  // ---------------------------------------------------------------- Sparkling
  s({
    id: 'champagne-brut',
    name: 'Champagne Brut',
    color: 'sparkling',
    grapeIds: ['chardonnay', 'pinot-noir'],
    regionId: 'champagne',
    profile: {
      appearance: 'lemon',
      intensity: [3, 4],
      sweetness: [1, 2],
      acidity: [5, 5],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['green-apple', 'lemon', 'brioche', 'toast', 'almond'],
    oak: 'none',
    difficulty: 2,
    story: [
      'Champagne Brut er referencen for mousserende vin: en blanding af Chardonnay, Pinot Noir og Pinot Meunier fra det kølige, nordfranske kridtlandskab, gæret anden gang på flasken og lagret på gæren (sur lie) i mindst 15 måneder, de bedste i mange år.',
      'Gærlagringen giver de karakteristiske noter af brioche, ristet brød og mandel oven på den høje syre og de grønne æbler. Brut betyder tør, under 12 gram sukker pr. liter.',
    ],
    sources: ['Wine Folly, champagne, https://winefolly.com/grapes/champagne/ (hentet 2026-09-17)'],
    note: 'Pinot Meunier is not in grapes.ts; the blend is listed as Chardonnay + Pinot Noir. Wine Folly gives 10–11.5% ABV, but Champagne is typically 12–12.5%; alcohol set to medium. Review.',
  }),
  s({
    id: 'prosecco',
    name: 'Prosecco Superiore',
    color: 'sparkling',
    grapeIds: ['glera'],
    regionId: 'conegliano-valdobbiadene',
    profile: {
      appearance: 'lemon-green',
      intensity: [3, 3],
      sweetness: [1, 2],
      acidity: [4, 4],
      tannin: null,
      alcohol: [2, 3],
      body: [1, 2],
      finish: [2, 2],
    },
    descriptorIds: ['green-apple', 'pear', 'melon', 'acacia'],
    oak: 'none',
    difficulty: 2,
    story: [
      'Prosecco er Italiens svar på mousserende hverdagsvin og laves på Glera med tankmetoden, hvor anden gæring sker i store trykbeholdere. Det bevarer druens friske frugt og giver en lettere, mindre gæragtig stil end Champagne.',
      'Grønt æble, pære og melon med blød mousse og moderat syre er kendetegnene. De fleste er brut eller extra dry, som trods navnet er lidt sødere.',
    ],
    sources: ['Wine Folly, prosecco, https://winefolly.com/grapes/prosecco/ (hentet 2026-09-17)'],
  }),
  s({
    id: 'cava',
    name: 'Cava',
    color: 'sparkling',
    grapeIds: ['macabeo', 'xarel-lo', 'parellada'],
    regionId: 'penedes',
    profile: {
      appearance: 'lemon',
      intensity: [2, 3],
      sweetness: [1, 2],
      acidity: [4, 5],
      tannin: null,
      alcohol: [2, 3],
      body: [2, 2],
      finish: [2, 3],
    },
    descriptorIds: ['quince', 'lime', 'green-apple', 'almond', 'toast'],
    oak: 'none',
    difficulty: 3,
    story: [
      'Cava laves som Champagne med anden gæring på flasken og mindst 9 måneder på gæren (18 for Reserva, 30 for Gran Reserva), men på de catalanske druer Macabeo, Xarel·lo og Parellada.',
      'Stilen ligger mellem Prosecco og Champagne: tør, med kvæde, lime og æble og en let nøddeagtig gærnote, men mindre brioche end Champagne og mere struktur end Prosecco.',
    ],
    sources: [
      'Wine Folly, cava, https://winefolly.com/grapes/cava/ (hentet 2026-09-17)',
      'Wikipedia, Cava (Spanish wine), https://en.wikipedia.org/wiki/Cava_(Spanish_wine) (hentet 2026-09-17)',
    ],
  }),

  // ---------------------------------------------------------------- Sweet
  s({
    id: 'tokaji-aszu',
    name: 'Tokaji Aszú',
    color: 'sweet',
    grapeIds: ['furmint'],
    regionId: 'tokaj',
    profile: {
      appearance: 'gold',
      intensity: [5, 5],
      sweetness: [5, 5],
      acidity: [5, 5],
      tannin: null,
      alcohol: [2, 3],
      body: [4, 5],
      finish: [5, 5],
    },
    descriptorIds: ['apricot', 'honey', 'orange-peel', 'dried-fruit', 'quince'],
    oak: 'light',
    difficulty: 3,
    story: [
      'Tokaji Aszú er Ungarns berømte søde vin, lavet ved at røre botrytiserede aszú-bær ud i most eller vin og gære blandingen langsomt. Antallet af puttonyos (kurve med aszú-bær pr. fad) angav traditionelt sødmen.',
      'Vinen er gylden til ravfarvet med abrikos, honning, appelsinskal og tørret frugt, og Furmints meget høje syre gør den frisk trods sødmen. Sammenlignet med Sauternes er den lettere i alkohol og skarpere i syren.',
    ],
    sources: [
      'Wikipedia, Tokaji, https://en.wikipedia.org/wiki/Tokaji (hentet 2026-09-17)',
      'Wine Folly, furmint, https://winefolly.com/grapes/furmint/ (hentet 2026-09-17)',
    ],
  }),
  s({
    id: 'moscato-d-asti',
    name: "Moscato d'Asti",
    color: 'sweet',
    grapeIds: ['muscat'],
    regionId: 'asti',
    profile: {
      appearance: 'lemon-green',
      intensity: [5, 5],
      sweetness: [4, 4],
      acidity: [3, 4],
      tannin: null,
      alcohol: [1, 1],
      body: [1, 2],
      finish: [2, 3],
    },
    descriptorIds: ['grape', 'peach', 'orange-blossom', 'honeysuckle', 'elderflower'],
    oak: 'none',
    difficulty: 2,
    story: [
      "Moscato d'Asti er en let perlende, sød vin på Moscato Bianco fra bakkerne omkring Asti. Gæringen afbrydes med kulde, så vinen beholder druens sukker og højst når 5,5 % alkohol.",
      'Den dufter umiskendeligt af friske druer, fersken og appelsinblomst og er sød, let og lav i alkohol. Kombinationen af meget høj aromaintensitet og meget lav alkohol gør den let at kende.',
    ],
    sources: [
      "Wikipedia, Moscato d'Asti, https://en.wikipedia.org/wiki/Moscato_d'Asti (hentet 2026-09-17)",
    ],
  }),

  // ---------------------------------------------------------------- Fortified
  s({
    id: 'port-lbv',
    name: 'Port (LBV)',
    color: 'fortified',
    grapeIds: ['touriga-nacional', 'tempranillo'],
    regionId: 'douro',
    profile: {
      appearance: 'ruby',
      intensity: [4, 5],
      sweetness: [4, 5],
      acidity: [3, 4],
      tannin: [4, 5],
      alcohol: [5, 5],
      body: [5, 5],
      finish: [4, 5],
    },
    descriptorIds: ['black-plum', 'blackberry', 'dried-fruit', 'chocolate', 'cinnamon'],
    oak: 'light',
    difficulty: 2,
    story: [
      'Portvin laves ved at tilsætte druebrændevin (aguardente) midt i gæringen, så gæren dør, sukkeret bliver i vinen, og alkoholen ender på 19–22 %. Druerne er Douros lokale sorter med Touriga Nacional og Tinta Roriz (Tempranillo) i spidsen.',
      'Late Bottled Vintage lagres 4–6 år på fad, før den aftappes, og giver noget af vintage-portvinens dybde uden årtiers flaskelagring. Stilen er mørk, sød og fyldig med sort blomme, tørret frugt, chokolade og kraftig tannin.',
    ],
    sources: [
      'Wine Folly, port, https://winefolly.com/grapes/port/ (hentet 2026-09-17)',
      'Wikipedia, Port wine, https://en.wikipedia.org/wiki/Port_wine (hentet 2026-09-17)',
    ],
    note: 'Tinta Roriz is mapped to the tempranillo entry (alias). Touriga Franca is not in grapes.ts.',
  }),
  s({
    id: 'fino-sherry',
    name: 'Fino Sherry',
    color: 'fortified',
    grapeIds: ['palomino'],
    regionId: 'jerez',
    profile: {
      appearance: 'lemon',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: null,
      alcohol: [5, 5],
      body: [2, 3],
      finish: [3, 4],
    },
    descriptorIds: ['almond', 'saline', 'lemon', 'nuts', 'green-apple'],
    oak: 'none',
    difficulty: 3,
    story: [
      'Fino er den lyseste og tørreste sherry. Vinen på Palomino forstærkes til cirka 15 % og lagres under et lag flor-gær, som beskytter den mod luften og giver den karakteristiske note af mandel, gærdej og salt.',
      'Solera-systemet, hvor yngre vin gradvist blandes i ældre fade, holder stilen ens fra år til år. Fino drikkes kold og ung, gerne til tapas.',
    ],
    sources: [
      'Wikipedia, Sherry, https://en.wikipedia.org/wiki/Sherry (hentet 2026-09-17)',
      'Wine Folly, sherry, https://winefolly.com/grapes/sherry/ (hentet 2026-09-17)',
    ],
    note: 'Wine Folly lists sherry acidity as high and sweetness off-dry across all styles; Fino is bone dry and the acidity is set to medium/medium+. Review.',
  }),

  // ---------------------------------------------------------------- Rosé
  s({
    id: 'provence-rose',
    name: 'Provence Rosé',
    color: 'rosé',
    grapeIds: ['grenache', 'syrah'],
    regionId: 'provence',
    profile: {
      appearance: 'salmon',
      intensity: [2, 3],
      sweetness: [1, 1],
      acidity: [4, 4],
      tannin: null,
      alcohol: [3, 3],
      body: [2, 2],
      finish: [2, 3],
    },
    descriptorIds: ['strawberry', 'redcurrant', 'grapefruit', 'dried-herbs'],
    oak: 'none',
    difficulty: 2,
    story: [
      'Provence Rosé er den lyse, tørre rosé, der har sat standarden for hele verden. Den laves typisk på Grenache, Cinsault, Syrah og Mourvèdre med kort skindkontakt, så farven bliver bleg laksefarvet.',
      'Stilen er let med frisk syre, jordbær, ribs, citrus og et strejf af urter. Den er skabt til at drikkes kold og ung.',
    ],
    sources: [
      'Wikipedia, Provence wine, https://en.wikipedia.org/wiki/Provence_wine (hentet 2026-09-17)',
    ],
    note: 'Cinsault and Mourvèdre are not in grapes.ts; the blend is listed as Grenache + Syrah.',
  }),
  s({
    id: 'tavel',
    name: 'Tavel',
    color: 'rosé',
    grapeIds: ['grenache'],
    regionId: 'tavel',
    profile: {
      appearance: 'pink',
      intensity: [3, 4],
      sweetness: [1, 1],
      acidity: [3, 4],
      tannin: [1, 2],
      alcohol: [4, 4],
      body: [3, 3],
      finish: [3, 3],
    },
    descriptorIds: ['strawberry', 'raspberry', 'dried-herbs', 'white-pepper', 'orange-peel'],
    oak: 'none',
    difficulty: 3,
    story: [
      'Tavel er den mørkeste og kraftigste af Frankrigs klassiske roséer og laves udelukkende som rosé, hovedsagelig på Grenache og Cinsault. Længere skindkontakt giver dybere farve, mere krop og lidt tannin.',
      'Sammenlignet med Provence er Tavel mørkere, fyldigere og højere i alkohol, med modne røde bær, urter og peber. Den kan tåle mad, som en let rødvin kan.',
    ],
    sources: ['Wikipedia, Tavel AOC, https://en.wikipedia.org/wiki/Tavel_AOC (hentet 2026-09-17)'],
    note: 'Cinsault is not in grapes.ts.',
  }),
]
